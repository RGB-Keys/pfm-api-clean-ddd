import { CreateIncomeUseCase } from '@/api/application/use-cases/client/create-income.use-case'
import { ClientNotFoundError } from '@/api/core/errors/domain/client/client-not-found-error'
import { CurrentUser } from '@/api/infra/http/auth/current-user-decorator'
import { JwtAuthGuard } from '@/api/infra/http/auth/jwt-auth.guard'
import type { UserPayload } from '@/api/infra/http/auth/jwt.strategy'
import { SWAGGER_TAGS } from '@/api/infra/http/swagger/tags/swagger-tags'
import { CreateIncomeDto } from '@/api/presentation/dtos/client/create-income/create-income.dto'
import { CreateIncomeResponseDto } from '@/api/presentation/dtos/client/create-income/create-income.response-dto'
import { IncomeSummaryPresenter } from '@/api/presentation/presenters/income/income-summary.presenter'
import {
	Body,
	Controller,
	HttpCode,
	HttpStatus,
	NotFoundException,
	Post,
	UseGuards,
} from '@nestjs/common'
import { ApiBearerAuth, ApiCreatedResponse, ApiTags } from '@nestjs/swagger'

@ApiTags(SWAGGER_TAGS.CLIENT)
@Controller('clients/incomes')
export class CreateIncomeController {
	constructor(private readonly createIncomeUseCase: CreateIncomeUseCase) {}

	@Post()
	@UseGuards(JwtAuthGuard)
	@ApiBearerAuth()
	@HttpCode(HttpStatus.CREATED)
	@ApiCreatedResponse({
		description: 'Income has been successfully added.',
		type: CreateIncomeResponseDto,
	})
	async handler(
		@Body() createIncomeDto: CreateIncomeDto,
		@CurrentUser() user: UserPayload,
	) {
		const clientId = user.sub

		const result = await this.createIncomeUseCase.execute({
			...createIncomeDto,
			clientId,
		})

		if (result.isFail()) {
			const error = result.value

			if (error instanceof ClientNotFoundError)
				throw new NotFoundException('Client not found.')

			throw new Error('Unhandled Error.')
		}

		return IncomeSummaryPresenter.toHttp(result.value.income)
	}
}
